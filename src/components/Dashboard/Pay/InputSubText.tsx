...
export default function InputSubText({
  ...
  const receiveText = useMemo(() => {
    const formatReceivedTickets = (wei: BigNumber) => {
      ...
      return formatWad(exchangeRate, { decimals: 0 })
    }
    ...
    const receivedTickets = formatReceivedTickets(
      (payIn === CURRENCY_ETH ? parseEther('1') : converter.usdToWei('1')) ??
        BigNumber.from(0),
    )
    return `${receivedTickets} ${tokenText}/${currencyName(payIn)}`
  }, [converter, payIn, tokenText, weiPayAmt, currentFC])
